...
export async function submitToQueueOwnerWithRetry(params: {
  timeoutMs?: number;
  ...
  while (Date.now() - startedAt < timeoutMs) {
    const ownerPromise = params.runSessionQueueOwner(params.ownerOptions);
    const remainingMs = Math.max(1, timeoutMs - (Date.now() - startedAt));
    const attemptDeadline = Date.now() + Math.min(1_000, remainingMs);

    while (Date.now() < attemptDeadline) {
      const result = await params.trySubmitToRunningOwner(params.submitOptions);
      if (result) {
        return { ownerPromise, result };
      }
      const settled = await Promise.race([
        ...
        new Promise<boolean>((resolve) => setTimeout(() => resolve(false), pollIntervalMs)),
      ]);
      if (settled) {
        break;
      }
  ...
  throw new Error(`Failed to submit queued task within ${timeoutMs}ms`);
}
