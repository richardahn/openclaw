...
describe("handleLineWebhookEvents", () => {
  ...
  it("skips group messages by default when requireMention is not configured", async () => {
    ...
    const event = createTestMessageEvent({
      message: { id: "m-default-skip", type: "text", text: "hi there", quoteToken: "qt-default" },
  ...
  it("records unmentioned group messages as pending history", async () => {
    ...
    >();
    const event = createTestMessageEvent({
      message: { id: "m-hist-1", type: "text", text: "hello history", quoteToken: "qt-hist-1" },
  ...
  it("skips group messages without mention when requireMention is set", async () => {
    const processMessage = vi.fn();
    const event = createTestMessageEvent({
      message: { id: "m-mention-1", type: "text", text: "hi there", quoteToken: "qt-mention-1" },
  ...
  it("does not apply requireMention gating to DM messages", async () => {
    const processMessage = vi.fn();
    const event = createTestMessageEvent({
      message: { id: "m-mention-dm", type: "text", text: "hi", quoteToken: "qt-mention-dm" },
